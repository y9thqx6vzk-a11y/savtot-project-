...
export default function Step4OutsideView({ isActive, isPast }: { isActive: boolean, isPast: boolean }) {
    ...
    return (
      <motion.div layout transition={transition} className="text-sm space-y-1">
        <div className="font-medium text-[#1d1d1f] dark:text-zinc-200 truncate">
          רפרנס דומה: {project.referenceProject || "ללא רפרנס"}
        </div>
        <div className="text-xs text-[#86868b] font-mono">
          תוכנן: {project.plannedDurationDays} ימים | לקח בפועל: {project.actualDurationDays} ימים | סטייה: <span className={project.optimismGapPercent > 20 ? 'text-[#1d1d1f] dark:text-white font-bold' : 'text-[#86868b]'}>+{project.optimismGapPercent}%</span>
        </div>
      </motion.div>
    );
  }
  ...
  return (
    <motion.div layout transition={transition} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="text-xs text-[#666] dark:text-zinc-400 font-light leading-relaxed border-r-2 border-[#1d1d1f] dark:border-white pr-3">
      ...
      {project.historicalBenchmarks && project.historicalBenchmarks.length > 0 && (
        <div className="p-4 bg-[#f5f3ee] dark:bg-zinc-950 border border-[#e5e1d6] dark:border-zinc-850 rounded-2xl flex items-center justify-between">
          <div>
            <div className="text-[11px] uppercase tracking-wider text-[#86868b] font-semibold">כיול על בסיס פרויקטי עבר</div>
            <div className="text-xs text-[#1d1d1f] dark:text-zinc-300 mt-0.5">
              ממוצע האופטימיות שלך מ-{project.historicalBenchmarks.length} פרויקטים קודמים הוא <span className="font-mono font-bold text-[#1d1d1f] dark:text-white">+{avgBias}%</span>
            </div>
          </div>
      ...
      <div className="space-y-2">
        ...
        <input 
          ...
          placeholder="שם של פרויקט דומה שאתה או אחרים בניתם..."
          value={project.referenceProject}
          onChange={(e) => updateProject({ referenceProject: e.target.value })}
        />
      </div>

      {/* שאלה: כמה זמן הם לקחו? */}
      <div className="grid grid-cols-2 gap-6">
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <label className="text-xs tracking-wider uppercase font-semibold text-[#86868b] block">כמה זמן תוכנן?</label>
            {plannedParsed && (
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <label className="text-xs tracking-wider uppercase font-semibold text-[#86868b] block">כמה זמן לקח בפועל?</label>
            {actualParsed && (
              <span className="text-[11px] font-mono text-[#555] dark:text-zinc-400">
      ...
      {/* שאלה: איזה ערך מוסף יש בפרויקט שלנו? */}
      <div className="space-y-2">
        <label className="text-xs tracking-wider uppercase font-semibold text-[#86868b] block">
        ...
        <textarea 
          ...
          rows={2}
          placeholder="במה הפתרון שלנו חד, פשוט ומועיל יותר? מהו היתרון הברור?"
          value={project.addedValueText || ""}
          onChange={(e) => updateProject({ addedValueText: e.target.value })}
        />
      </div>

      {/* שאלה: מה אפשר ללמוד מהפרויקטים האלו? */}
      <div className="space-y-2">
        <label className="text-xs tracking-wider uppercase font-semibold text-[#86868b] block">
          מה אפשר ללמוד מהפרויקטים האלו? (לקחים למניעת תקלות)
        </label>
        <textarea 
          className="w-full bg-white/70 dark:bg-zinc-950/70 border border-[#e2ded5] dark:border-zinc-800 focus:border-[#1d1d1f] dark:focus:border-zinc-500 rounded-xl p-3 text-sm text-[#1d1d1f] dark:text-zinc-200 focus:outline-none transition-colors resize-none placeholder:text-[#a8a49c] dark:placeholder:text-zinc-700 shadow-sm"
          rows={2}
          placeholder="איזה טעויות נעשו שם שלא נחזור עליהן כאן? (למשל: היעדר חוצץ בצווארי בקבוק)..."
      ...
      {/* Calculated Gap Badge */}
      {project.optimismGapPercent > 0 && (
        <div className="bg-[#f7f5ef] dark:bg-zinc-950 p-4 border border-[#e5e1d6] dark:border-zinc-850 rounded-2xl flex justify-between items-center">
          <div>
            <div className="text-[11px] uppercase tracking-wider text-[#86868b] font-semibold">פער סטיית זמנים</div>
            <div className="text-xl font-mono text-[#1d1d1f] dark:text-white">+{project.optimismGapPercent}% מעבר למתוכנן</div>
          </div>
          <span className="text-xs font-mono text-[#86868b]">
            תוספת באפר מומלצת לשלב 6: כ-{Math.ceil((plannedParsed?.days || 14) * (project.optimismGapPercent / 100))} ימים
          </span>
        </div>
      )}
